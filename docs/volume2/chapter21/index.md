...
1. **压缩与泛化**：一个模型在训练集上达到零误差，但在测试集上误差很大（过拟合）。另一个模型在训练集上误差是 5%，但在测试集上误差也是 5%（良好泛化）。用 MDL 的语言，描述这两个模型的差异：哪个压缩了更多信息？哪个找到了更短的描述？

2. **归纳偏置的显化**：线性回归、决策树、神经网络三种学习算法，分别隐含了什么样的归纳偏置？用"什么样的规律优先"来描述。这三种偏置，各自在什么情况下会失败？

3. **不可计算性的代价**：Kolmogorov 复杂度不可计算。这意味着"找到最优归纳偏置"是不可判定的。这对机器学习的理论上限意味着什么？是否存在某类问题，理论上可学习，但最优学习算法本身不可计算？
