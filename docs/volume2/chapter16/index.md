...
考虑以下三个命题，判断在线性逻辑中，它们是否成立。对于每个命题，给出直觉解释。

1. $A \otimes B \vdash A$（如果同时拥有 $A$ 和 $B$，可以单独推出 $A$）
2. $A \,\&\, B \vdash A$（如果拥有"$A$ 或 $B$ 的选择权"，可以推出 $A$）
3. $A \multimap B, A \multimap B \vdash A \multimap B$（两份"$A$ 变为 $B$"的能力，可以合并为一份？）

第1个和第2个的对比揭示了 $\otimes$ 和 $\&$ 的本质差异。第3个是收缩规则在特殊情况下的投影——想想为什么它在线性逻辑里会失败。
