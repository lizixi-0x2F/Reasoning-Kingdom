...
一个医学检测对某种疾病的灵敏度（sensitivity）是 90%，特异度（specificity）是 95%。也就是说，患病者有 90% 概率检测阳性，健康者有 95% 概率检测阴性。该疾病的人群患病率是 1%。

1. 某人检测结果为阳性，他实际患病的概率是多少？（先用直觉估计，再用贝叶斯定理计算。）
2. 如果进行第二次独立检测，结果仍为阳性，患病概率变为多少？
3. 如果这个人来自一个高风险群体，患病率是 10%，同样检测阳性，患病概率又是多少？

注意第1个问题的答案可能让你吃惊。第2个和第3个问题是在演示：先验的更新如何依赖数据，而先验的选择如何依赖背景知识。这正是贝叶斯推断的核心：推理从不在真空中进行。
