...
1. **归约的方向**：如果 $A \leq_p B$（$A$ 可以多项式时间归约到 $B$），且 $B \in \mathsf{P}$，那么 $A \in \mathsf{P}$。但如果 $A \leq_p B$ 且 $A$ 是 NP 完全的，这对 $B$ 意味着什么？分两种情况讨论：$B \in \mathsf{NP}$ 和 $B \notin \mathsf{NP}$。

2. **停机问题的变体**：以下哪些问题是可判定的，哪些不是？给出直觉理由。
   - 给定程序 $P$ 和输入 $I$，$P$ 在 $I$ 上是否在 $10^{100}$ 步内停机？
   - 给定程序 $P$，$P$ 是否对所有输入都停机？
   - 给定程序 $P$，$P$ 是否对某个输入停机？

3. **证明长度与复杂度**：第14章指出，验证一个命题逻辑证明是多项式时间可完成的。这意味着"命题逻辑定理证明"这个问题在 NP 里吗？如果答案是"是"，那么 SAT 是 NP 完全的这件事，对于"找到命题逻辑证明"这个任务，有什么推论？

第3个问题把第14章和第19章焊在了一起：**证明的发现，在最坏情况下，和 SAT 一样难**。这不是偶然，而是逻辑和计算在最深处的一次相遇。
