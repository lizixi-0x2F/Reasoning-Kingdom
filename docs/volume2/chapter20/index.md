...
1. **可采纳性的代价**：构造一个搜索问题，使得可采纳的 $h_1$ 需要展开 100 个节点才找到最优解，而不可采纳的 $h_2$ 只需展开 5 个节点就找到了同一个解。这说明可采纳性保证的是什么，不保证的是什么？

2. **近似比的传递性**：若对问题 $A$ 有 $\rho_A$-近似算法，且 $A$ 可以多项式时间归约到 $B$（归约保持代价的常数倍关系），那么对 $B$ 能推出什么近似保证？归约方向和近似比之间的关系，与第19章的复杂度归约有何异同？

3. **PAC 与贝叶斯的关系**：第17章的贝叶斯推断和本章的 PAC 学习，都在处理"从有限数据泛化"的问题。PAC 保证在最坏的分布下成立，贝叶斯保证在给定先验下成立。哪种保证更强？哪种更有用？在什么情况下两者会给出截然不同的答案？
