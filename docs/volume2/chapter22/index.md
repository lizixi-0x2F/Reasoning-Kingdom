...
1. **Curry-Howard 的具体实例**：考虑命题 $A \to (B \to A)$（蕴含的弱化）。在类型论里，它对应什么类型？写出一个居住在这个类型里的程序（函数），并验证它"证明"了这个命题。

2. **不动点与递归**：$Y$ 组合子满足 $Y\,f = f\,(Y\,f)$。用这个等式，展开 $Y\,(\lambda x.\, x + 1)$ 的前几步。发生了什么？这说明不动点和"无限循环"之间有什么关系？

3. **自指的边界**：回到哥德尔句 $G$（"我在系统 $\mathcal{F}$ 中不可证"）。$G$ 是第15章里逻辑自指的例子；$Y$ 组合子是本章里计算自指的例子。两者在结构上有什么共同点，又有什么根本区别？哥德尔句制造了一个悖论，而 $Y$ 组合子制造了一个无限循环——同样是自指，为什么结果不同？
