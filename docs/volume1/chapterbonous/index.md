...
而更深的困境还藏在前缀和本身的物理意义里。cumsum 状态压缩在做什么？它把所有历史 token 的外积 $k_t \otimes v_t$ 不断叠加进同一个矩阵 $S$——这正是第9章番外里讲过的现代霍普菲尔德网络的能量最小化：存储模式的加权叠加，给定查询时做归一化检索。线性注意力不是在逃离霍普菲尔德，而是在用前缀和重新实现它，只是去掉了 softmax 的全局归一化。状态矩阵 $S$ 就是霍普菲尔德的权重矩阵 $W = \sum_\mu p^\mu (p^\mu)^\top$，只不过是在线更新的版本。走了一圈，回到了1982年。这不是巧合，这是联想记忆这个数学结构的引力——足够深的探索必然会落回它身上。
